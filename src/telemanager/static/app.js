...
const elements = {
  ...
  accountsConfigStatus: document.querySelector("#accountsConfigStatus"),
  ...
  toggleApiHash: document.querySelector("#toggleApiHash"),
  ...
  pendingAccountSummary: document.querySelector("#pendingAccountSummary"),
...
let safetySettingsLoaded = false
async function loadSafetySettings() {
  try {
    const payload = await request("/api/settings/safety")
    applySafetySettings(payload.settings)
    safetySettingsLoaded = true
  } catch (error) {
    showToast(error.message)
  }
}

async function ensureSafetySettings() {
  if (safetySettingsLoaded) return
...
function startActivityPolling() {
  ...
  activityPollTimer = window.setInterval(loadActivity, 10000)
}
...
function openView(viewName) {
  ...
  if (viewName === "actions" || viewName === "settings") {
    ensureSafetySettings()
  }
}
...
async function refresh() {
  ...
  state.apiConfigured = Boolean(config.api_hash_configured)
  const configMessage = config.api_hash_configured
    ? `Configured with API ID ${config.api_id}.`
    : "API settings are not configured yet."
  elements.configStatus.textContent = configMessage
  elements.configForm.elements.api_id.value = config.api_id || ""
  elements.configForm.elements.api_hash.placeholder = config.api_hash_configured
    ? "Leave blank to keep saved hash"
    : "Telegram API hash"
  if (elements.accountsConfigStatus)
    elements.accountsConfigStatus.textContent = configMessage
  updateMetrics()
  populateDialogAccountSelect()
  renderAccountTables()
  renderActionAccounts()
  renderActionQueue()
  renderPendingAccountSummary()
}

function renderPendingAccountSummary() {
  if (!elements.pendingAccountSummary) return
  const account = state.accounts.find(
    (item) => item.id === state.pendingAccountId
  )
  elements.pendingAccountSummary.classList.toggle("empty", !account)
  if (!account) {
    elements.pendingAccountSummary.textContent = "Request a code to begin."
    return
  }
    ...
    : account.status === "password_pending"
      ...
      : account.status === "login_pending"
        ...
        : account.authorized
          ? "Ready"
          : "Needs login"
  elements.pendingAccountSummary.replaceChildren()
  const title = document.createElement("strong")
  title.textContent = account.label || account.session_name
  const detail = document.createElement("span")
  detail.textContent = `${account.phone || "Phone not recorded"} · ${status}`
  elements.pendingAccountSummary.append(title, detail)
  if (account.last_error) {
    const error = document.createElement("span")
    error.textContent = account.last_error
    error.className = "danger-text"
    elements.pendingAccountSummary.append(error)
  }
}
...
async function fetchDialogs(accountId = elements.dialogAccountSelect.value) {
  ...
  try {
    elements.dialogFetchStatus.textContent = "Fetching dialogs from Telegram..."
    const limit = Number(
...
function renderDialogs() {
  ...
  if (!dialogs.length) {
    ...
    dialogs.forEach((dialog) => {
      const row = document.createElement("tr")
      const target = dialog.username ? `@${dialog.username}` : String(dialog.id)
      const selectCell = document.createElement("td")
      const checkbox = document.createElement("input")
      checkbox.type = "checkbox"
...
async function renameAccountPrompt(account) {
  ...
  if (!label) return showToast("Account label cannot be empty.")
...
async function renameSessionPrompt(account) {
  ...
  if (!sessionName) return showToast("Session filename cannot be empty.")
...
function renderActionPreview(preview) {
  ...
  if (preview.unauthorized_count > 0) {
    const unauthorized = document.createElement("span")
    unauthorized.className = "preview-unauthorized"
    unauthorized.textContent = `${preview.authorized_count} ready · ${preview.unauthorized_count} will be skipped (not logged in)`
    card.append(unauthorized)
  }
...
elements.clearQueueButton.addEventListener("click", async () => {
  if (!state.actionQueue.length) return showToast("Queue is already empty.")
  const confirmed = await askModal({
...
elements.savePresetButton.addEventListener("click", async () => {
  ...
  if (name === null) return
  if (!name) return showToast("Preset name cannot be empty.")
  try {
...
elements.runQueueButton.addEventListener("click", async () => {
  ...
  await withBusyButton(elements.runQueueButton, "Running...", async () => {
    try {
...
elements.toggleApiHash.addEventListener("click", () => {
  const input = elements.configForm.elements.api_hash
  const reveal = input.type === "password"
  input.type = reveal ? "text" : "password"
  elements.toggleApiHash.textContent = reveal ? "Hide" : "Show"
})

elements.configForm.addEventListener("submit", async (event) => {
  event.preventDefault()
  try {
    const form = new FormData(elements.configForm)
    const apiId = String(form.get("api_id") || "").trim()
    const apiHash = String(form.get("api_hash") || "").trim()
    if (!apiId) return showToast("API ID is required.")
    if (!state.apiConfigured && !apiHash)
      return showToast("API hash is required.")
    const payload = await request("/api/config", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ api_id: apiId, api_hash: apiHash }),
    })
    elements.configForm.elements.api_id.value = payload.api_id
    elements.configForm.elements.api_hash.value = ""
    await refresh()
...
elements.loginForm.addEventListener("submit", async (event) => {
  event.preventDefault()
  await withBusyButton(event.submitter, "Sending...", async () => {
    try {
      if (!state.apiConfigured) {
        showToast("Save Telegram API settings before requesting a login code.")
        openView("settings")
        return
      }
      const body = new FormData(elements.loginForm)
      body.set("phone", String(body.get("phone") || "").trim())
      body.set("label", String(body.get("label") || "").trim())
      const payload = await request("/api/accounts/login", {
        method: "POST",
        body,
      })
      if (payload.account.authorized) {
        state.pendingAccountId = null
        await refresh()
        showToast("This account already has a ready local session.")
        return
      }
      ...
      elements.challengeHint.textContent = `Code requested for ${payload.account.phone}. Check Telegram, then enter it below.`
      await refresh()
      elements.codeForm.elements.code.focus()
      showToast("Login code requested. Check Telegram on that phone number.")
...
elements.codeForm.addEventListener("submit", async (event) => {
  ...
  await withBusyButton(event.submitter, "Verifying...", async () => {
    try {
      ...
      if (payload.account.status === "password_pending") {
        ...
        elements.challengeHint.textContent =
          "Telegram accepted the code. Enter the 2FA password to finish login."
        elements.passwordForm.elements.password.focus()
      } else {
...
elements.passwordForm.addEventListener("submit", async (event) => {
  ...
  await withBusyButton(event.submitter, "Verifying...", async () => {
    try {
      ...
      state.pendingAccountId = null
      await refresh()
      showToast("2FA confirmed. The local session is ready.")
      logActivity("Account login complete", "Saved ready session")
    } catch (error) {
      showToast(error.message)
    }
...
elements.sessionImportForm.addEventListener("submit", async (event) => {
  event.preventDefault()
  await withBusyButton(event.submitter, "Importing...", async () => {
    try {
