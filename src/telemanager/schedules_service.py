...
class SchedulerService:
            ...
            for step_index, step in enumerate(schedule["queue"]["steps"]):
                ...
                for account_id in step["account_ids"]:
                    ...
                    for target in step["targets"]:
                        ...
                        chat_coverage = await self._reconcile_chat(client, native_chats, key, target, text, desired)
    ...
    async def _reconcile_chat(
        ...
        for fire in desired:
            ...
            if fire in covered:
                continue
            message_id = await create_scheduled_text(client, target, text, fire)
            tracked[str(message_id)] = iso(fire)
            covered.add(fire)
            room -= 1
        entry["ids"] = tracked
